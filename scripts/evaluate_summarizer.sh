...
if [ -z "$DATASET" ] || [ -z "$CONFIG" ]; then
    ...
    echo "💡 Usage: bash scripts/evaluate_summarizer.sh [summe|tvsum|all] [config_path]"
    exit 1
fi
...
if [ "$DATASET" == "summe" ]; then
    python src/evaluate.py --dataset summe --config ${CONFIG}

# 评测 TVSum
elif [ "$DATASET" == "tvsum" ]; then
    python src/evaluate.py --dataset tvsum --config ${CONFIG}
