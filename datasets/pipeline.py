...
class DecordDecode:
    ...
    def __call__(self, results):
        ...
        # Generate frame index mapping in order (带防撞墙容错机制)
        frame_dict = {}
        for idx in np.unique(frame_inds):
            try:
                # 正常尝试读取
                frame_dict[idx] = container[idx].asnumpy()
            except Exception:
                # 如果这帧坏了，往前回退一帧
                try:
                    frame_dict[idx] = container[max(0, idx - 1)].asnumpy()
                except Exception:
                    # 如果前一帧也坏了，直接拿第0帧来保底！
                    frame_dict[idx] = container[0].asnumpy()
